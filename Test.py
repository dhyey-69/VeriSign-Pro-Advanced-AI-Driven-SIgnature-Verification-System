...
model.load_state_dict(load(open('Models/model_epoch_20', 'rb'), map_location=device))
...
def compute_accuracy_roc(predictions, labels):
    ...
    return max_acc, d_optimal # Here d_optimal is threshold value
...
def test():
    ...
    total_batches = len(test_dataset) // 8 + (1 if len(test_dataset) % 8 != 0 else 0)
